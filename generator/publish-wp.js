...
function discover() {
  ...
  /* The Orange County page is the site's front page and the top of the silo.
     It is hand-maintained rather than generated, so it is not in communities.json
     and has to be added explicitly. It publishes first: every city page links up
     to it. Skipped when --only or a single city is requested. */
  if (!onlyCity) {
    const oc = path.join(ROOT, 'orange-county-sales-page', 'index.html');
    if (fs.existsSync(oc)) {
      sorted.unshift({ file: oc, rel: '', slug: COUNTY_SLUG, parentPath: null, type: 'county', depth: 0, isCounty: true });
    }
  }
  ...
  /* --only=<slug> publishes one page, for a contained first test. Its parents
     ...
     --only=county targets the front page. */
  const only = (args.find(a => a.startsWith('--only=')) || '').split('=')[1];
  if (!only) return sorted;
  if (only === 'county') return sorted.filter(p => p.isCounty);
  return sorted.filter(p => p.slug === only);
}
...
/* Where the county page lives on WordPress. Its content is the site's front page,
   so links to it go to the root. */
const OC_PAGE = process.env.OC_PAGE || '/';

/* The county page gets a real slug rather than being written straight into the
   existing front page. That page is one Fabian built in Elementor, and CLAUDE.md
   puts hand-built Elementor pages off limits. Publishing beside it and then
   pointing page_on_front at the new page leaves his original intact and makes the
   switch a single reversible setting rather than an overwrite.

   WordPress serves whatever page_on_front names at "/" and redirects its slug URL
   there, so the canonical stays the bare domain either way. */
...
/* The county page is the one page that LIVES in orange-county-sales-page/, so its
   paths have a different shape from every generated page and rewriteAssets() does
   not touch them:

     assets   "assets/x.png"  'viz-photos/'  url(video/y.mp4)   — bare, same folder
     links    "../irvine/"                                       — up one, then across

   The bare form includes a JS string ("const DIR = 'viz-photos/'") that builds
   image URLs at runtime, which is why the quote/paren is part of the match rather
   than a word boundary — see F-16. Anchoring on the opening delimiter also stops
   the rule firing inside words like "video/" in prose. */
function rewriteCountyPaths(html) {
  return html
    .replace(/(["'(])(assets|viz-photos|video)\//g, `$1${ASSET_BASE}/$2/`)
...
function buildContent(x, isCounty) {
  const html = `<!-- GENERATED by generator/publish-wp.js — do not edit in WordPress.
...
(async () => {
  ...
  for (const p of pages) {
    const wpPageUrl = p.isCounty ? `${WP_URL}/` : `${WP_URL}/${p.rel}/`;
    const x = rewriteCanonical(extract(p.file), wpPageUrl);
    const content = buildContent(x, p.isCounty);
    ...
    if (!LIVE) {
      ...
      console.log(`  ${p.type.padEnd(10)} ${p.isCounty ? '/  (front page)' : '/' + p.rel + '/'}`);
    ...
    /* Point the site at the county page. Opt-in, because it changes what every
       visitor to the bare domain sees. The previous front page is only detached,
       never edited — put its id back to undo. */
    if (p.isCounty && SET_FRONT) {
      const before = await wp('/settings');
      const prev = before.page_on_front;
      if (prev === saved.id) {
        console.log(`           already the front page`);
      } else {
        await wp('/settings', {
          method: 'POST',
          body: JSON.stringify({ show_on_front: 'page', page_on_front: saved.id })
        });
        console.log(`           front page  ${prev} -> ${saved.id}   (undo: set page_on_front back to ${prev})`);
      }
    }
  }
